...
class Registration extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->order 		= ['participant_id', 'participant_name', 'participant_wa', 'participant_nip', 'participant_nik', 'participant_unit_bisnis', 'participant_department', 'participant_keberangkatan', 'addon', 'participant_qr'];
		$this->search 		= ['participant_id', 'participant_name', 'participant_wa', 'participant_nip', 'participant_nik', 'participant_unit_bisnis', 'participant_department', 'participant_keberangkatan', 'addon', 'participant_qr'];
	...
	public function view()
	{
		...
		foreach ($res as $row) {
			...
			$col[] = ($row->participant_keberangkatan == 1 ? 'Bus (dari kedoya)' : ($row->participant_keberangkatan == 2 ? 'Kendaraan Sendiri (Motor)' : 'Kendaraan Sendiri (Mobil)') );
			$col[] = ($row->addon ? date('d/m/Y H:i:s', strtotime($row->addon)) : '-' );
			$col[] = '<img src="'.base_url().'assets/public/registran/'.$row->participant_nip.'/'.$row->participant_qr.'">';
			$data[] = $col;
			$a++;
		}
		$output = array(
